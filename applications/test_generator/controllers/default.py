...
def lab_1461_3():
    ...
    if form.process().accepted:
        ...
        task1 = list(map(
            lambda a: LI(
                P(
                    'Имеется монохромное изображение в разрешении ',
                    a[0],
                    'х',
                    a[1],
                    'px  с количеством оттенков ',
                    a[2],
                    '. Задание: Найти размер изображения в Гбайтах'
                )
            ), (
                (rnd.randint(640, 2048), rnd.randint(480, 1200), rnd.randint(2, 256)),
            )
        ))
        ...
        task4 = list(map(
            lambda a: LI(
                P(
                    'Вы скачиваете с сайта 4K видео при этом ваш браузер показывает, что размер файла равен ',
                    a[0],
                    ' Гб. Задание: найти минимальное время скачивания файла при условии, что ваша максимальныя скорось загрузки данных из сети равна ',
                    a[1],
                    'Мбит/с '
                )
            ), (
                (rnd.randint(1, 10) + round(rnd.random(), 2), rnd.randint(1, 120)),
            )
        ))
        ...
        task5 = list(map(
            lambda a: LI(
                P(
                    'Вы загружаете фотографии на сайт, при этом вы знаете, что всего у вас  ',
                    a[0],
                    ' фотографий и каждая весит по ',
                    a[1],
                    ' Кб. Задание: найти время загрузки файлов на сайт при условии, что ваша скорость скачивания ',
                    a[2],
                    'Мбит/с, а загрузки (отдачи) ',
                    a[3],
                    'Мбит/c'
                )
            ), (
                (rnd.randint(10, 50), rnd.randint(16, 4096), rnd.randint(1, 120) + round(rnd.random(),2), rnd.randint(0,50) + round(rnd.random(),2)),
            )
        ))
        ...
        return dict(content=DIV(
            H3(u'Задание'),
            OL(task1 + task2 + task3 + task4 + task5),
            A(BUTTON(u'Обновить'), _href='')
        ))
    else:
        return dict(content=form)